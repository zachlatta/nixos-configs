# Pulled from https://github.com/NixOS/nixpkgs/blob/0cfd08f4881bbfdaa57e68835b923d4290588d98/nixos/modules/services/x11/desktop-managers/plasma5.nix
# 
# Need to pull out to create a version of KDE without kscreen (which is overwriting my custom monitor config)
...
let
  ...
  cfg = xcfg.desktopManager.custom-plasma5;
...

{
  options = {
    ...
    services.xserver.desktopManager.custom-plasma5 = {
      enable = mkOption {
        type = types.bool;
        default = false;
  ...
  imports = [
    (mkRemovedOptionModule [ "services" "xserver" "desktopManager" "custom-plasma5" "enableQt4Support" ] "Phonon no longer supports Qt 4.")
    (mkRenamedOptionModule [ "services" "xserver" "desktopManager" "custom-kde5" ] [ "services" "xserver" "desktopManager" "plasma5" ])
  ];
    ...
    (mkIf cfg.enable {
      ...
      environment.systemPackages =
        ...
        [
          ...
          kinfocenter
          kmenuedit
          # kscreen - Removed so monitors can be manually configured with Xrandr
          kscreenlocker
          ksysguard
          kwayland
          kwin
